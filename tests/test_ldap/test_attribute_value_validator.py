...
class TestOrganizationalUnitName:
    ...
    def test_valid_names(self, validator: AttributeValueValidator) -> None:
        ...
        valid_names = ["IT Department", "Sales", "Marketing-Team", "HR_Department", "Department123"]
        for name in valid_names:
            assert validator.is_value_valid(self._entity_type_name, "name", name)

    def test_invalid_names_with_space_at_start(self, validator: AttributeValueValidator) -> None:
        """Test organizational unit names starting with space."""
        invalid_names = [" IT", " Sales", "  Marketing"]
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "name", name)

    def test_invalid_names_with_hash_at_start(self, validator: AttributeValueValidator) -> None:
        """Test organizational unit names starting with hash."""
        invalid_names = ["#IT", "#Sales", "#Marketing"]
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "name", name)

    def test_invalid_names_with_space_at_end(self, validator: AttributeValueValidator) -> None:
        """Test organizational unit names ending with space."""
        invalid_names = ["IT ", "Sales  ", "Marketing   "]
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "name", name)

    def test_invalid_names_with_forbidden_symbols(self, validator: AttributeValueValidator) -> None:
        """Test organizational unit names with forbidden symbols."""
        invalid_names = ['IT"Dept', "Sales,Team", "Marketing+", "HR\\Group", "Dept<1>", "Team;A", "Group=B"]
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "name", name)
...
class TestGroupName:
    ...
    def test_valid_names(self, validator: AttributeValueValidator) -> None:
        ...
        valid_names = ["Administrators", "Users", "Power_Users", "Group-123", "TeamA"]
        for name in valid_names:
            assert validator.is_value_valid(self._entity_type_name, "name", name)
    ...
    def test_invalid_names_with_forbidden_symbols(self, validator: AttributeValueValidator) -> None:
        ...
        invalid_names = ['Admins"Group', "Users,Team", "Power+Users", "Group\\A", "Team<1>", "Users;B", "Group=C"]
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "name", name)
...
class TestUserName:
    ...
    def test_valid_names(self, validator: AttributeValueValidator) -> None:
        ...
        valid_names = ["John Doe", "Jane_Smith", "User-123", "Administrator", "User.Name"]
        for name in valid_names:
            assert validator.is_value_valid(self._entity_type_name, "name", name)
    ...
    def test_invalid_names_with_forbidden_symbols(self, validator: AttributeValueValidator) -> None:
        ...
        invalid_names = ['John"Doe', "Jane,Smith", "User+123", "Name\\Test", "User<1>", "John;Doe", "User=Name"]
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "name", name)
...
class TestUserSAMAccountName:
    ...
    def test_valid_sam_account_names(self, validator: AttributeValueValidator) -> None:
        """Test valid sAMAccountName values."""
        valid_names = ["jdoe", "john.doe", "user123", "admin_user", "test-user"]
        for name in valid_names:
            assert validator.is_value_valid(self._entity_type_name, "sAMAccountName", name)

    def test_invalid_sam_account_names_with_forbidden_symbols(self, validator: AttributeValueValidator) -> None:
        ...
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "sAMAccountName", name)

    def test_invalid_sam_account_names_ending_with_dot(self, validator: AttributeValueValidator) -> None:
        """Test sAMAccountName ending with dot."""
        invalid_names = ["user.", "john.doe.", "admin."]
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "sAMAccountName", name)

    def test_invalid_sam_account_names_with_control_chars(self, validator: AttributeValueValidator) -> None:
        """Test sAMAccountName with control characters."""
        invalid_names = ["user\x00name", "user\x01name", "user\x1fname", "user\x7fname"]
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "sAMAccountName", name)

    def test_invalid_sam_account_names_with_at_symbol(self, validator: AttributeValueValidator) -> None:
...
class TestComputerName:
    ...
    def test_valid_names(self, validator: AttributeValueValidator) -> None:
        ...
        valid_names = ["WORKSTATION01", "Server-2024", "PC_LAB_123", "Desktop"]
        for name in valid_names:
            assert validator.is_value_valid(self._entity_type_name, "name", name)
    ...
    def test_invalid_names_with_forbidden_symbols(self, validator: AttributeValueValidator) -> None:
        ...
        invalid_names = ['PC"01', "Server,01", "Work+Station", "PC\\01", "Server<1>", "PC;01", "Computer=01"]
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "name", name)
...
class TestComputerSAMAccountName:
    ...
    def test_valid_sam_account_names(self, validator: AttributeValueValidator) -> None:
        ...
        valid_names = ["WORKSTATION01$", "SERVER-2024$", "PC_LAB$"]
        for name in valid_names:
            assert validator.is_value_valid(self._entity_type_name, "sAMAccountName", name)
    ...
    def test_invalid_sam_account_names_with_control_chars(self, validator: AttributeValueValidator) -> None:
        ...
        invalid_names = ["PC\x00NAME$", "PC\x01NAME$", "PC\x1fNAME$", "PC\x7fNAME$"]
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "sAMAccountName", name)

    def test_invalid_sam_account_names_with_spaces_and_dots(self, validator: AttributeValueValidator) -> None:
        """Test computer sAMAccountName with spaces and dots."""
        invalid_names = ["PC 01$", "SERVER 2024$", "WORK.STATION$", "PC.01$"]
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "sAMAccountName", name)

    def test_invalid_sam_account_names_only_numbers(self, validator: AttributeValueValidator) -> None:
        """Test computer sAMAccountName that are only numbers."""
        invalid_names = ["123", "456789", "0"]
        for name in invalid_names:
            assert not validator.is_value_valid(self._entity_type_name, "sAMAccountName", name)

    def test_invalid_sam_account_names_starting_with_number(self, validator: AttributeValueValidator) -> None:
...
class TestNoValidationRules:
    ...
    def test_attributes_without_rules_always_valid(self, validator: AttributeValueValidator) -> None:
        ...
        for entity_type, property_name, value in test_cases:
            assert validator.is_value_valid(entity_type, property_name, value)
