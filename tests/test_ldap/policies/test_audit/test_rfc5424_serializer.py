...
from ldap_protocol.policies.audit.events.service_senders.rfc5424_serializer import RFC5424Serializer


@pytest.fixture
def serializer() -> RFC5424Serializer:
    """Create serializer instance."""
    return RFC5424Serializer(app_name="TestApp", facility="authpriv")


@pytest.mark.parametrize(
    ("facility", "severity", "expected_severity"),
    [("kernel", 5, 5), ("user", 3, 11), ("authpriv", 6, 86), ("local0", 7, 135), ("local7", 2, 186)],
)
def test_format_priority(facility: str, severity: int, expected_severity: int) -> None:
...
@pytest.mark.parametrize("invalid_severity", [-1, 8, 10, 100])
def test_format_priority_invalid_severity(serializer: RFC5424Serializer, invalid_severity: int) -> None:
...
@pytest.mark.parametrize(
    ("hostname", "expected"), [("server01.example.com", "server01.example.com"), ("a" * 300, "a" * 255)]
)
def test_format_hostname(serializer: RFC5424Serializer, hostname: str, expected: str) -> None:
...
def test_format_field(serializer: RFC5424Serializer, value: str | None, max_length: int, expected: str) -> None:
...
@pytest.mark.parametrize(
    ("data", "expected_result"), [({}, "-"), ({"username": "admin"}, '[audit@32473 username="admin"]')]
)
def test_format_structured_data(serializer: RFC5424Serializer, data: dict, expected_result: str) -> None:
    """Test _format_structured_data with various inputs."""
    result = serializer._format_structured_data(data)  # noqa: SLF001
    assert result == expected_result


def test_format_structured_data_multiple_params(serializer: RFC5424Serializer) -> None:
    """Test _format_structured_data with multiple parameters."""
    data = {"username": "admin", "ip": "192.168.1.100", "action": "login"}
    result = serializer._format_structured_data(data)  # noqa: SLF001
...
def test_sanitize_param_name(serializer: RFC5424Serializer, input_name: str, expected: str) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('Test "quote" and \\backslash and ]bracket', r"Test \"quote\" and \\backslash and \]bracket"),
    ],
)
def test_escape_param_value(serializer: RFC5424Serializer, input_value: str, expected: str) -> None:
    """Test _escape_param_value with various special characters."""
    result = serializer._escape_param_value(input_value)  # noqa: SLF001
    assert result == expected


@pytest.mark.parametrize(("input_msg", "expected"), [("User logged in", " \ufeffUser logged in"), (None, ""), ("", "")])
def test_format_message(serializer: RFC5424Serializer, input_msg: str | None, expected: str) -> None:
