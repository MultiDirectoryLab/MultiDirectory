...
async def test_service_update_audit_policy(audit_service: AuditService) -> None:
    """Test updating an audit policy."""
    old_policy = (await audit_service.get_policies())[0]

    name = "Test Policy"
    new_policy = AuditPolicyDTO(name=name, is_enabled=True, severity=old_policy.severity)
    ...
    for policy in await audit_service.get_policies():
        ...
        pytest.fail(f"Policy with id {old_policy.get_id()} not found in updated policies.")


@pytest.mark.asyncio
@pytest.mark.usefixtures("setup_session")
async def test_service_update_audit_policy_with_non_existing_id(audit_service: AuditService) -> None:
    """Test updating an audit policy."""
    latest_policy = (await audit_service.get_policies())[-1]

    possible_id = latest_policy.get_id() + 1

    new_policy = AuditPolicyDTO(name="Test Policy", is_enabled=True, severity=latest_policy.severity)

    with pytest.raises(AuditNotFoundError, match=f"Policy with id {possible_id} not found."):
        await audit_service.update_policy(possible_id, new_policy)


@pytest_asyncio.fixture(scope="function")
@pytest.mark.usefixtures("setup_session")
async def audit_destination(audit_service: AuditService) -> AuditDestinationDTO:
    ...
    destination = next((dest for dest in destinations if dest.name == new_destination.name), None)
...
async def test_service_update_audit_destination(
    audit_service: AuditService, audit_destination: AuditDestinationDTO
) -> None:
    ...
    await audit_service.update_destination(audit_destination.id, updated_destination)
...
async def test_service_delete_audit_destination(
    ...
    destinations = await audit_service.get_destinations()
    assert all(dest.id != audit_destination.id for dest in destinations), "Destination was not deleted successfully."
...
async def test_service_create_existing_audit_destination(
    ...
    with pytest.raises(AuditAlreadyExistsError, match="Audit destination already exists"):
...
async def test_service_update_non_existing_audit_destination(
    ...
    with pytest.raises(AuditNotFoundError, match=f"Destination with id {provision_id} not found."):
        await audit_service.update_destination(provision_id, audit_destination)
...
async def test_service_delete_non_existing_audit_destination(
    ...
    assert audit_destination.id is not None, "Destination ID is None."
    with pytest.raises(AuditNotFoundError, match=f"Destination with id {audit_destination.id + 1} not found."):
