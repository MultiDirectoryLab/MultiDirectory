...
async def test_reset_success(
    ...
    """Test reset user password history endpoint."""
    user_name = "testuser"
    response = await http_client.post(
        f"/user/reset_password_history/{user_name}",
    )
    ...
    user_password_history_use_cases.reset.assert_called_once()
    assert response.status_code == status.HTTP_200_OK


@pytest.mark.asyncio
async def test_reset_unauthorized(
    http_client_with_login_perm: AsyncClient,
    user_password_history_use_cases: Mock,
) -> None:
    """Test reset user password history endpoint without permissions."""
    user_name = "testuser"
    ...
    # NOTE to user_password_history_use_cases.reset returned Mock, not wrapper  # noqa: E501
    user_password_history_use_cases._perm_checker = None  # noqa: SLF001
    user_password_history_use_cases.reset.assert_not_called()
