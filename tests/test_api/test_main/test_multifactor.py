...
@pytest.mark.parametrize(
    ...
    [
        ...
        (httpx.ConnectTimeout("Connection timed out"), True, MultifactorAPI.MFAConnectError),
        # 2. httpx.ReadTimeout => False
        (httpx.ReadTimeout("Read timed out"), False, None),
        # 3. status_code=401 => raise MFAMissconfiguredError
        (httpx.Response(status_code=401), True, MultifactorAPI.MFAMissconfiguredError),
        # 4. status_code=500 => raise MultifactorError
        (httpx.Response(status_code=500), True, MultifactorAPI.MultifactorError),
        # 5. status_code=200, 'model.status' != "Granted" => False
        (
            httpx.Response(status_code=200, json={"model": {"status": "Denied"}}, request=httpx.Request("POST", "")),
            False,
            None,
        ),
        # 6. status_code=200, 'model.status' == "Granted" => True
        (
            httpx.Response(status_code=200, json={"model": {"status": "Granted"}}, request=httpx.Request("POST", "")),
            True,
            None,
        ),
    ],
