...
@pytest.mark.parametrize(
    ("group_dn", "directory_dn", "error_message"),
    [
        (
            "cn=test,dc=md,dc=test",
            "cn=non-existent,cn=groups,dc=md,dc=test",
            "Directory with DN 'cn=non-existent,cn=groups,dc=md,dc=test'"
            " not found.",
        ),
        (
            "cn=non-existent,dc=md,dc=test",
            "cn=domain admins,cn=groups,dc=md,dc=test",
            "Group with DN 'cn=non-existent,dc=md,dc=test' not found.",
        ),
    ],
)
async def test_api_set_primary_group_non_existent_directory(
    http_client: AsyncClient,
    group_dn: str,
    directory_dn: str,
    error_message: str,
) -> None:
    """Test API for setting primary group for non-existent directory."""
    response = await http_client.post(
        "/entry/set_primary_group",
        json={
    ...
    data = response.json()
    assert data["resultCode"] == LDAPCodes.NO_SUCH_OBJECT
    assert data["errorMessage"] == error_message
