...
async def test_tree_creation(
    ...
    response = await http_client.post(
        "/kerberos/setup/tree",
        json={"mail": "777@example.com", "krbadmin_password": krbadmin_pw},
    )
...
async def test_tree_collision(http_client: AsyncClient) -> None:
    ...
    response = await http_client.post(
        "/kerberos/setup/tree",
        json={"mail": "777@example.com", "krbadmin_password": "Password123"},
    )
...
async def test_setup_call(
    http_client: AsyncClient, kadmin: Mock, creds: TestCreds
) -> None:
    """Test setup args.
...
async def test_status_change(
    http_client: AsyncClient, creds: TestCreds
) -> None:
    """Test setup args.
...
async def test_bind_create_user(
    http_client: AsyncClient, kadmin: AbstractKadmin, settings: Settings
) -> None:
...
async def test_add_princ(
    ...
    response = await http_client.post(
        ...
        json={"primary": "host", "instance": "12345"},
    )
    kadmin_args = kadmin.add_principal.call_args.args  # type: ignore
    assert response.status_code == status.HTTP_200_OK
...
async def test_rename_princ(
    ...
    response = await http_client.patch(
        ...
        json={"principal_name": "name", "principal_new_name": "nname"},
    )
...
async def test_change_princ(
    ...
    response = await http_client.patch(
        ...
        json={"principal_name": "name", "new_password": "pw123"},
    )
...
async def test_delete_princ(
    ...
    """
    response = await http_client.request(
        "delete", "/kerberos/principal/delete", json={"principal_name": "name"}
    )
    assert response.status_code == status.HTTP_200_OK
