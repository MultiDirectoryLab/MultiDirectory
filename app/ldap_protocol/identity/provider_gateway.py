...
class IdentityProviderGateway:
    ...
    async def get_user_permissions(self, role_ids: list[int]) -> AuthorizationRules:
        permissions = await self.session.scalars(select(qa(Role.permissions)).where(qa(Role.id).in_(role_ids)))
