...
class DirectoryDAO:
    ...
    async def delete_configuration_dir(self) -> None:
        ...
        await self.__session.execute(delete(Directory).where(qa(Directory.name) == CONFIGURATION_DIR_NAME))
        await self.__session.flush()

    async def create_directory(self, name: str, is_system: bool, parent_dir: Directory) -> Directory:
        """Create a Directory and return it with id populated."""
        directory = Directory(is_system=is_system, object_class="", name=name)
        directory.groups = []
    ...
    async def get_all_without_entity_type(self) -> list[Directory]:
        ...
        result = await self.__session.scalars(
            ...
            .options(selectinload(qa(Directory.attributes)), selectinload(qa(Directory.entity_type)))
        )
        return list(result.all())

    async def get_configuration_dir(self) -> Directory:
        """Get configuration directory."""
        result = await self.__session.execute(select(Directory).where(qa(Directory.name) == CONFIGURATION_DIR_NAME))
        return result.scalar_one()

    async def get_all_dir_ids_by_entity_type_name(self, name: str) -> list[int]:
        """Get all Directory IDs by Entity Type name."""
        result = await self.__session.scalars(
            select(qa(Directory.id)).join(qa(Directory.entity_type)).where(qa(EntityType.name) == name)
        )
        return list(result.all())

    async def bind_entity_type(self, directory: Directory, entity_type_id: int | None) -> None:
        """Ensure the Directory.entity_type relationship is loaded."""
        directory.entity_type_id = entity_type_id
        await self.__session.flush()
        await self.__session.refresh(directory, attribute_names=["entity_type"])
