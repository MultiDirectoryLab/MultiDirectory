...
class AttributeDAO:
    ...
    async def add_directory_name_attribute(self, directory_id: int, attribute_dto: AttributeDTO) -> None:
    ...
    async def add_attributes_from_dto(self, directory_id: int, attributes: tuple[AttributeDTO, ...]) -> None:
        ...
        for attribute_dto in attributes:
            for value in attribute_dto.values:
                ...
                self.__session.add(
                    Attribute(directory_id=directory_id, name=attribute_dto.name, value=value, bvalue=None)
                )
