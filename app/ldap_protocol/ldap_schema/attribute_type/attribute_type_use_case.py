...
from ldap_protocol.ldap_schema.dto import AttributeDTO, AttributeTypeDTO, DirCreateDTO
from ldap_protocol.ldap_schema.exceptions import AttributeTypeAlreadyExistsError
...
class AttributeTypeUseCase(AbstractService):
    ...
    def __init__(
        ...
        self.__attribute_type_system_flags_use_case = attribute_type_system_flags_use_case
    ...
    async def get(self, name: str) -> AttributeTypeDTO[int]:
        ...
        dto.object_class_names = await self.__object_class_dao.get_object_class_names_include_attribute_type(dto.name)
        return dto
    ...
    async def create(self, dto: AttributeTypeDTO) -> None:
        ...
        if not dto.ldap_display_name:
            dto.ldap_display_name = f"{dto.name[0].lower()}{dto.name.replace('-', '')[1:]}"
        ...
        _dto = DirCreateDTO(
            ...
            attributes=(
                ...
                AttributeDTO(name=Names.LDAP_DISPLAY_NAME, values=[str(dto.ldap_display_name)]),
                AttributeDTO(name=Names.SYNTAX, values=[str(dto.syntax)]),
                AttributeDTO(name=Names.SINGLE_VALUE, values=[str(dto.single_value)]),
                AttributeDTO(name=Names.NO_USER_MODIFICATION, values=[str(dto.no_user_modification)]),
                AttributeDTO(name=Names.SYSTEM_FLAGS, values=[str(dto.system_flags)]),
                AttributeDTO(name=Names.IS_INCLUDED_ANR, values=[str(dto.is_included_anr)]),
            ),
            is_system=dto.is_system,
        )
        try:
            await self.__directory_create_use_case.create_dir(dto=_dto, parent_dir=self.__parent_dir)
    ...
    async def is_attr_replicated(self, name: str) -> bool:
        ...
        dto = await self.__attribute_type_dao.get(name)
        return self.__attribute_type_system_flags_use_case.is_attr_replicated(dto)

    async def set_attr_replication_flag(self, name: str, need_to_replicate: bool) -> None:
        """Set replication flag in systemFlags."""
        dto = await self.get(name)
        dto = self.__attribute_type_system_flags_use_case.set_attr_replication(dto, need_to_replicate)
        await self.__attribute_type_dao.update_sys_flags(dto.name, dto)
    ...
    PERMISSIONS: ClassVar[dict[str, AuthorizationRules]] = {
        ...
        delete_all_by_names.__name__: AuthorizationRules.ATTRIBUTE_TYPE_DELETE_ALL_BY_NAMES,
    }
