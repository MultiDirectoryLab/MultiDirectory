...
_convert_model_to_dto = get_converter(
    ...
    recipe=[
        ...
        link_function(lambda _: "", P[AttributeTypeDTO].ldap_display_name),
...
class ObjectClassDAOLegacy:
    ...
    async def create(self, dto: ObjectClassCreateDTO) -> None:
    ...
    async def delete_all_dirs(self) -> None:
        objcls_subq = (
            select(qa(EntityType.id)).where(qa(EntityType.name) == EntityTypeNames.OBJECT_CLASS).scalar_subquery()
        )
        await self.__session.execute(delete(Directory).where(qa(Directory.entity_type_id).in_(objcls_subq)))
