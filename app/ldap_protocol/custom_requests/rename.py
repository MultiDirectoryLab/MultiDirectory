...
from ldap_protocol.ldap_requests import ModifyDNRequest as LDAPModifyDNRequest, ModifyRequest as LDAPModifyRequest
...
class RenameRequest(BaseModel):
    ...
    async def _modify_dn_request(self, container: AsyncContainer, entry: str, newrdn: str) -> LDAPResult:
        modify_dn_request = LDAPModifyDNRequest(entry=entry, newrdn=newrdn, deleteoldrdn=True, new_superior=None)
    ...
    async def _modify_request(self, container: AsyncContainer) -> LDAPResult:
        modify_request = LDAPModifyRequest(object=self._new_object, changes=self.changes)
    ...
    async def handle_api(self, container: AsyncContainer) -> LDAPResult:
        ...
        """
        modify_dn_response = await self._modify_dn_request(container, self.object, self.newrdn)
        ...
        if not modify_response or modify_response.result_code != 0:
            await self._modify_dn_request(container, self._new_object, self._oldrdn)
