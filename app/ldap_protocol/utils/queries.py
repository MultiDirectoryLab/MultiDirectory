...
from sqlalchemy.orm import InstrumentedAttribute, contains_eager, joinedload, selectinload
...
from .helpers import create_integer_hash, dn_is_base_directory, ft_now, validate_entry


@base_directories_cache
async def get_base_directories(session: AsyncSession) -> list[Directory]:
    """Get base domain directories."""
    result = await session.execute(select(Directory).filter(qa(Directory.parent_id).is_(None)))
    res = []
...
async def get_user(session: AsyncSession, name: str) -> User | None:
    ...
    return await session.scalar(
        select(User).join(qa(User.directory)).options(policies).where(get_filter_from_path(name))
    )


async def get_directories(
    dn_list: list[GRANT_DN_STRING], session: AsyncSession, excluded_group: Group | None = None
) -> list[Directory]:
    ...
    query = select(Directory).filter(or_(*paths)).options(joinedload(qa(Directory.group)))

    if excluded_group:
        excluded_subq = select(directory_memberships_table.c.directory_id).where(
            directory_memberships_table.c.group_id == excluded_group.id
        )
        query = query.where(~exists(excluded_subq.where(directory_memberships_table.c.directory_id == Directory.id)))
...
async def extend_group_membership(group: Group, members_to_add: list[Directory], session: AsyncSession) -> None:
    """Extend group memberships."""
    await session.execute(
        insert(directory_memberships_table).values(
            [{"group_id": group.id, "directory_id": directory.id} for directory in members_to_add]
        )
    )


async def clear_group_membership(group: Group, session: AsyncSession) -> None:
    """Clear group memberships."""
    await session.execute(
        directory_memberships_table.delete().where(directory_memberships_table.c.group_id == group.id)
    )


async def remove_disallowed_group_members(
    group: Group, allowed_members: list[Directory], session: AsyncSession
) -> None:
...
async def remove_from_group_membership(group: Group, members_to_remove: list[Directory], session: AsyncSession) -> None:
...
async def get_directory_by_rid(rid: str, session: AsyncSession) -> Directory | None:
    query = (
        select(Directory)
        .join(Attribute, qa(Attribute.directory_id) == qa(Directory.id))
        .options(selectinload(qa(Directory.attributes)), joinedload(qa(Directory.group)))
        .where(qa(Attribute.name) == "objectSid", qa(Attribute.value).endswith(f"-{rid}"))
    )
    return await session.scalar(query)


async def groups_include_primary_rid(session: AsyncSession, groups: list[Group], primary_group_id: str) -> bool:
...
async def get_groups(dn_list: list[str], session: AsyncSession) -> list[Group]:
    ...
    query = (
        ...
        .options(
            ...
            joinedload(qa(Group.directory)).selectinload(qa(Directory.attributes)),
        )
    )
...
async def get_group(dn: str | GRANT_DN_STRING, session: AsyncSession) -> Group:
    ...
    query = (
        select(Group)
        .join(qa(Group.directory), isouter=True)
        .options(joinedload(qa(Group.directory)).selectinload(qa(Directory.attributes)))
    )

    query = query.filter(get_filter_from_path(dn)) if validate_entry(dn) else query.filter_by(name=dn)
...
async def set_user_logon_attrs(user: User, session: AsyncSession, tz: ZoneInfo) -> None:
    ...
    await session.execute(
        update(Attribute)
        .values({"value": ft_now()})
        .filter_by(directory_id=user.directory_id, name="pwdLastSet", value="-1")
    )
    await session.execute(update(User).values({"last_logon": datetime.now(tz=tz)}).filter_by(id=user.id))
...
def get_path_filter(
    path: list[str], *, column: ColumnElement | Column | InstrumentedAttribute = directory_table.c.path
) -> ColumnElement:
...
def get_filter_from_path(dn: str, *, column: Column | InstrumentedAttribute = directory_table.c.path) -> ColumnElement:
...
def get_domain_object_class(domain: Directory) -> Iterator[Attribute]:
    ...
    for value in ["domain", "top", "domainDNS"]:
        yield Attribute(name="objectClass", value=value, directory_id=domain.id)


async def create_group(
    name: str, attribute_value_validator: AttributeValueValidator, session: AsyncSession
    ...
    query = select(Directory).filter(get_filter_from_path("cn=Groups," + base_dn_list[0].path_dn))

    parent = (await session.scalars(query)).one()

    dir_ = Directory(object_class="", name=name, parent_id=parent.id)
    session.add(dir_)
    await session.flush()
    ...
    await session.refresh(instance=dir_, attribute_names=["attributes", "user"], with_for_update=None)
    if not attribute_value_validator.is_directory_valid(dir_):
        raise AttributeValueValidatorError("Invalid directory attributes values")
...
async def add_lock_and_expire_attributes(session: AsyncSession, directory: Directory, tz: ZoneInfo) -> None:
    ...
    session.add_all(
        [
            Attribute(name="nsAccountLock", value="true", directory_id=directory.id),
            Attribute(name="shadowExpire", value=str(absolute_date), directory_id=directory.id),
        ]
    )


async def get_principal_directory(session: AsyncSession, principal_name: str) -> Directory | None:
    ...
    """
    return await session.scalar(
        select(Directory).filter_by(name=principal_name).options(selectinload(qa(Directory.attributes)))
    )


async def set_or_update_primary_group(
    directory_dn: GRANT_DN_STRING, group_dn: GRANT_DN_STRING, session: AsyncSession
) -> None:
    ...
    """
    directory = await session.scalar(select(Directory).filter(get_filter_from_path(directory_dn)))
    ...
    if not is_member:
        raise ValueError(f"Directory '{directory_dn}' is not a member of group '{group_dn}'.")

    updated_attribute = await session.scalar(
        update(Attribute)
        .values(value=group.directory.relative_id)
        .where(qa(Attribute.name) == "primaryGroupID", qa(Attribute.directory_id) == directory.id)
        .returning(qa(Attribute.directory_id))
    )

    if not updated_attribute:
        session.add(Attribute(name="primaryGroupID", directory_id=directory.id, value=group.directory.relative_id))

    await session.commit()


async def get_group_path_dn_by_primary_group_id(primary_group_id: int, session: AsyncSession) -> str:
    ...
    query = (
        ...
        .where(qa(Attribute.name) == "objectSid", qa(Attribute.value).endswith(f"-{primary_group_id}"))
    )

    directory = await session.scalar(query)
    if directory is None:
        raise ValueError(f"No group found with primaryGroupID '{primary_group_id}'.")
