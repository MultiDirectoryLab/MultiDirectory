...
class SyslogSender(AuditDestinationSenderABC):
    ...
    service_name: AuditDestinationServiceType = AuditDestinationServiceType.SYSLOG
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.__rfc_serializer = RFC5424Serializer(app_name=self.DEFAULT_APP_NAME, facility="authpriv")

    async def _send_udp(self, message: str) -> None:
        """Send UDP."""
        transport, _ = await asyncio.wait_for(
            asyncio.get_event_loop().create_datagram_endpoint(
                lambda: asyncio.DatagramProtocol(), remote_addr=(self._destination.host, self._destination.port)
            ),
    ...
    async def _send_tcp(self, message: str) -> None:
        ...
        _, writer = await asyncio.wait_for(
            asyncio.open_connection(host=self._destination.host, port=self._destination.port),
            timeout=self.DEFAULT_TIMEOUT,
        )
    ...
    async def send(self, event: NormalizedAuditEvent) -> None:
        ...
        syslog_message = self.__rfc_serializer.serialize(
            event=event, structured_data=structured_data, syslog_version=self.SYSLOG_VERSION
        )
        ...
        if self._destination.protocol == AuditDestinationProtocolType.UDP:
            ...
            raise NotImplementedError(f"Unsupported protocol: {self._destination.protocol}")

        try:
            await callback(syslog_message)
        except TimeoutError as te:
            logger.error(f"Timeout during syslog {self._destination.name} send: {te}")
            raise
        except (OSError, ConnectionRefusedError, ConnectionError) as ce:
            logger.error(f"Failed to send syslog {self._destination.name}: {ce}")
            raise
        except Exception as e:
            logger.error(f"Error during syslog {self._destination.name} send: {e}")
            raise
