    ...
    return await session.scalar(
        select(
            exists().where(  # type: ignore
                groups_table.c.mfa_policies.contains(policy),
                groups_table.c.users.contains(user),
            ),
        ),
    )
...
async def is_user_group_valid(
    ...
    query = (
        ...
        .where(groups_table.c.users.contains(user))
        .where(groups_table.c.policies.contains(policy))
        .limit(1)
    )
