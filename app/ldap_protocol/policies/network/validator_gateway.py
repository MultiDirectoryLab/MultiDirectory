...
class NetworkPolicyValidatorGateway:
    ...
    def _build_base_query(self, ip: IPv4Address | IPv6Address, protocol_type: ProtocolType) -> Select:
        ...
        query = (
            ...
            .options(selectinload(qa(NetworkPolicy.groups)), selectinload(qa(NetworkPolicy.mfa_groups)))
            .filter(
        ...
        if user_group_ids is not None:
            query = query.filter(
                or_(
                    ...
                    qa(NetworkPolicy.groups).any(qa(Group.id).in_(user_group_ids)),
                )
            )
    ...
    async def check_mfa_group(self, policy: NetworkPolicy, user: User) -> bool:
        ...
        return await self._session.scalar(
            select(exists().where(qa(Group.mfa_policies).contains(policy), qa(Group.users).contains(user)))  # type: ignore
        )
