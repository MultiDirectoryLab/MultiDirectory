...
class SetupGateway:
    ...
    async def is_setup(self) -> bool:
        ...
        """
        query = select(exists(Directory).where(qa(Directory.parent_id).is_(None)))
        retval = await self._session.scalars(query)
        return retval.one()

    async def setup_enviroment(self, *, data: list, is_system: bool = True, domain: Directory) -> None:
        """Create directories and users for enviroment."""
        async with self._session.begin_nested():
            self._session.add(
                NetworkPolicy(
                    name="Default open policy", netmasks=[IPv4Network("0.0.0.0/0")], raw=["0.0.0.0/0"], priority=1
                )
            )
            ...
            await self._session.refresh(instance=domain, attribute_names=["attributes"], with_for_update=None)

            entity_type = await self._entity_type_use_case.get(EntityTypeNames.DOMAIN)
            await self._directory_dao.bind_entity_type(domain, entity_type.id if entity_type else None)
            if not self._attribute_value_validator.is_directory_valid(domain):
                raise ValueError("Invalid directory attribute values during environment setup")
            await self._session.flush()

        try:
            for unit in data:
                await self.create_dir(unit, is_system=is_system, domain=domain, parent=domain)
    ...
    async def create_base_domain(self, dn: str = "multifactor.dev") -> Directory:
        """Create base domain."""
    ...
    async def create_dir(self, data: dict, is_system: bool, domain: Directory, parent: Directory | None = None) -> None:
        """Create data recursively."""
        dir_ = Directory(is_system=is_system, object_class=data["object_class"], name=data["name"])
        dir_.groups = []
        ...
        self._session.add(Attribute(name=dir_.rdname, value=dir_.name, directory_id=dir_.id))

        if "objectSid" in data:
            await self._object_sid_use_case.add(directory_id=dir_.id, rid=int(data["objectSid"]))
        ...
        if "attributes" in data:
            attrs = chain(data["attributes"].items(), [("objectClass", [dir_.object_class])])
        ...
        if "organizationalPerson" in data:
            ...
            user = User(
                ...
                mail=user_data["mail"],
                password=self._password_utils.get_password_hash(user_data["password"]),
            )
            self._session.add(user)
            await self._session.flush()
            self._session.add(Attribute(directory_id=dir_.id, name="homeDirectory", value=f"/home/{user.uid}"))
        ...
        await self._session.refresh(instance=dir_, attribute_names=["attributes", "user"], with_for_update=None)

        entity_type = None
        if entity_type_name := data.get("entity_type_name"):
            entity_type = await self._entity_type_use_case.get(entity_type_name)
        entity_type_id = entity_type.id if entity_type else None
        await self._entity_type_use_case.attach_entity_type_to_directory(
            directory=dir_, is_system_entity_type=True, entity_type_id=entity_type_id
        )
        ...
        if "children" in data:
            for n_data in data["children"]:
                await self.create_dir(n_data, is_system=is_system, domain=domain, parent=dir_)
    ...
    async def _get_group(self, name: str) -> Group:
        ...
        retval = await self._session.scalars(
            ...
            .filter(qa(Directory.name) == name, qa(Directory.object_class) == "group")
        )
        return retval.one()
