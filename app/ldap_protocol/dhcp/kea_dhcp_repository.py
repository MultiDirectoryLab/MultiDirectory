...
from .dataclasses import DHCPLease, DHCPOptionData, DHCPPool, DHCPReservation, DHCPSubnet
from .dtos import KeaDHCPAPILeaseRequest, KeaDHCPAPIReservationRequest, KeaDHCPAPISubnetRequest, KeaDHCPBaseAPIRequest
from .enums import KeaDHCPCommands, KeaDHCPResultCodes
from .exceptions import DHCPAPIError, DHCPConflictError, DHCPEntryNotFoundError, DHCPUnsupportedError
from .retorts import (
    add_lease_retort,
...
class KeaDHCPAPIRepository(DHCPAPIRepository):
    ...
    def _validate_api_response(response: httpx.Response) -> None:
        ...
        if response.status_code != 200:
            raise DHCPAPIError(f"Failed to communicate with DHCP API: {response.text}")
    ...
    async def _make_request(self, data: dict[str, Any], return_response: bool = False) -> httpx.Response | None:
    ...
    async def create_subnet(self, subnet_dto: DHCPSubnet) -> None:
        ...
        data = add_subnet_retort.dump(
            KeaDHCPAPISubnetRequest(command=KeaDHCPCommands.SUBNET4_ADD, subnet4=[subnet_dto])
        )
    ...
    async def update_subnet(self, subnet: DHCPSubnet) -> None:
        ...
        data = update_subnet_retort.dump(
            KeaDHCPAPISubnetRequest(command=KeaDHCPCommands.SUBNET4_UPDATE, subnet4=[subnet])
        )
    ...
    async def delete_subnet(self, subnet_id: int) -> None:
        ...
        data = delete_subnet_retort.dump(
            KeaDHCPAPISubnetRequest(command=KeaDHCPCommands.SUBNET4_DEL, subnet4=DHCPSubnet(id=subnet_id))
        )
    ...
    async def list_subnets(self) -> list[DHCPSubnet]:
        """List all subnets."""
        data = list_subnet_retort.dump(KeaDHCPBaseAPIRequest(command=KeaDHCPCommands.SUBNET4_LIST))
        response = await self._make_request(data, return_response=True)

        return (
            [
                DHCPSubnet(id=item["id"], subnet=item["subnet"])
                for item in response.json()[0].get("arguments").get("subnets", [])
            ]
    ...
    async def get_subnet_by_id(self, subnet_id: int) -> DHCPSubnet:
        ...
        data = get_subnet_retort.dump(
            KeaDHCPAPISubnetRequest(command=KeaDHCPCommands.SUBNET4_GET, subnet4=DHCPSubnet(id=subnet_id))
        )

        response = await self._make_request(data, return_response=True)

        subnet_data = response.json()[0].get("arguments").get("subnet4")[0] if response else {}

        return DHCPSubnet(
            id=subnet_data.get("id"),
            subnet=subnet_data.get("subnet"),
            pools=[DHCPPool(pool=pool["pool"]) for pool in subnet_data.get("pools", [])],
            valid_lifetime=subnet_data.get("valid-lifetime"),
            option_data=[
                DHCPOptionData(name=option["name"], data=option["data"])
    ...
    async def create_lease(self, lease: DHCPLease) -> None:
        """Create a new lease."""
        data = add_lease_retort.dump(KeaDHCPAPILeaseRequest(command=KeaDHCPCommands.LEASE4_ADD, lease=lease))
    ...
    async def release_lease(self, ip_address: IPv4Address) -> None:
        ...
        data = release_lease_retort.dump(
            KeaDHCPAPILeaseRequest(command=KeaDHCPCommands.LEASE4_DEL, lease=DHCPLease(ip_address=ip_address))
        )
    ...
    async def list_leases_by_subnet_id(self, subnet_ids: list[int]) -> list[DHCPLease]:
        """List all leases."""
        data = list_leases_retort.dump(KeaDHCPBaseAPIRequest(command=KeaDHCPCommands.LEASE4_LIST, arguments=subnet_ids))
        response = await self._make_request(data, return_response=True)

        leases_data = response.json()[0].get("arguments").get("leases") if response else None
    ...
    async def get_lease_by_hw_address(self, hw_address: str) -> DHCPLease:
        ...
        data = get_lease_by_hw_address_retort.dump(
            KeaDHCPAPILeaseRequest(
                command=KeaDHCPCommands.LEASE4_GET_BY_HW_ADDRESS, lease=DHCPLease(mac_address=hw_address)
            )
        )

        response = await self._make_request(data, return_response=True)

        lease_data = response.json()[0].get("arguments").get("leases")[0] if response else {}
    ...
    async def get_lease_by_hostname(self, hostname: str) -> DHCPLease:
        ...
        data = get_lease_by_hostname_retort.dump(
            KeaDHCPAPILeaseRequest(command=KeaDHCPCommands.LEASE4_GET_BY_HOSTNAME, lease=DHCPLease(hostname=hostname))
        )
    ...
    async def create_reservation(self, reservation: DHCPReservation) -> None:
        ...
        data = add_reservation_retort.dump(
            KeaDHCPAPIReservationRequest(command=KeaDHCPCommands.RESERVATION_ADD, arguments=reservation)
        )
    ...
    async def update_reservation(self, reservation: DHCPReservation) -> None:
        ...
        data = add_reservation_retort.dump(
            KeaDHCPAPIReservationRequest(command=KeaDHCPCommands.RESERVATION_UPDATE, arguments=reservation)
        )
    ...
    async def delete_reservation(self, reservation: DHCPReservation) -> None:
        ...
        data = delete_reservation_retort.dump(
            KeaDHCPAPIReservationRequest(command=KeaDHCPCommands.RESERVATION_DEL, arguments=reservation)
        )
    ...
    async def list_reservations(self, subnet_id: int) -> list[DHCPReservation]:
        ...
        data = get_all_reservations_retort.dump(
            KeaDHCPAPIReservationRequest(
                command=KeaDHCPCommands.RESERVATION_LIST, arguments=DHCPReservation(subnet_id=subnet_id)
            )
        )
        response = await self._make_request(data, return_response=True)

        reservations_data = (response.json()[0].get("arguments").get("hosts")) if response else []
    ...
    async def write_config(self) -> None:
        ...
        data = base_retort.dump(
            KeaDHCPBaseAPIRequest(command=KeaDHCPCommands.CONFIG_WRITE, arguments={"filename": "kea-dhcp4.conf"})
        )
    ...
    async def set_config(self, config: dict[str, Any]) -> None:
        ...
        data = base_retort.dump(KeaDHCPBaseAPIRequest(command=KeaDHCPCommands.CONFIG_SET, arguments=config))
