...
from ldap_protocol.dns.dto import DNSForwardZoneDTO, DNSMasterZoneDTO, DNSRecordDTO, DNSRRSetDTO, DNSSettingsDTO
...
class BindToPDNSMigrationUseCase:
    ...
    def __init__(self, pdns_manager: PowerDNSManager, dns_settings: DNSSettingsDTO) -> None:
    ...
    def _strip_record_name(self, record_name: str, zone_name: str) -> str:
        ...
        logger.debug(f"Stripping record name '{record_name}' for zone '{zone_name}'")
        if record_name.startswith(("\\032", "\\@")) and record_name != "\\@":
            record_name = record_name.removeprefix("\\032").removeprefix("\\@")
        elif record_name == "\\@":
            record_name = zone_name
        return record_name if not record_name.startswith(".") else record_name[1:]

    def parse_bind_config_file(self) -> tuple[list[DNSMasterZoneDTO], list[DNSForwardZoneDTO]]:
        ...
        with open(os.path.join(self.bind_config_files_dir, "named.conf.local")) as f:
            for line in f:
                line = line.strip()
                ...
                if "type master" in line:
                    master_zones.append(DNSMasterZoneDTO(id=zone_name, name=zone_name))
                elif "type forward" in line:
                    forward_zone = DNSForwardZoneDTO(id=zone_name, name=zone_name)
                elif "forwarders" in line and forward_zone:
                    forwarders_part = line.split("forwarders")[1]
                    forwarders = [
                        f
                        for f in forwarders_part.strip(";").strip(" ").strip("{").strip("}").strip(" ").split(";")[:-1]
                    ]
    ...
    def parse_zones_records(self, master_zones: list[DNSMasterZoneDTO]) -> list[DNSMasterZoneDTO]:
        ...
        for zone in master_zones:
            ...
            zone_file_path = os.path.join(self.bind_zone_file_dir, f"{zone.name}.zone")
            try:
                zone_obj = dns.zone.from_file(zone_file_path, origin=zone.name, relativize=False)
            except FileNotFoundError:
                logger.error(f"Zone file for zone {zone.name} not found, skipping...")
                continue
            ...
            for name, ttl, rdata in zone_obj.iterate_rdatas():
                try:
                    ...
                    logger.warning(f"Unsupported DNS record type {rdata.rdtype.name} in zone '{zone.name}'")
                    continue

                zone_rrsets.append(
                    DNSRRSetDTO(
                        name=self._strip_record_name(name.to_text(), zone.name),
                        type=record_type,
                        records=[DNSRecordDTO(content=rdata.to_text(), disabled=False)],
                        ttl=ttl,
                    )
                )
    ...
    async def get_bind_zones(self) -> tuple[list[DNSMasterZoneDTO], list[DNSForwardZoneDTO]]:
        """Get zones from BIND."""
    ...
    async def migrate_from_bind(self) -> None:
        ...
        for master_zone in master_zones:
            await self.pdns_manager.create_master_zone(master_zone, is_empty=True)
            for rrset in master_zone.rrsets:
                await self.pdns_manager.create_record(master_zone.name, rrset)
    ...
    def is_migration_needed(self) -> bool:
        ...
        return not (
            ...
            and os.path.exists(os.path.join(self.bind_config_files_dir, "migrated"))
