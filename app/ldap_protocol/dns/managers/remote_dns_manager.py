...
from ldap_protocol.dns.exceptions import DNSConnectionError, DNSNotImplementedError
...
class RemoteDNSManager(AbstractDNSManager):
    ...
    async def _send(self, action: Message) -> None:
        ...
        if self._dns_settings.tsig_key is not None:
            action.use_tsig(keyring=TsigKey("zone.", self._dns_settings.tsig_key), keyname="zone.")
    ...
    async def setup(self, dns_settings: DNSSettingsDTO, is_migration: bool = False) -> None:  # noqa: ARG002
    ...
    async def create_record(self, zone_id: str, record: DNSRRSetDTO) -> None:
        ...
        action.add(record.name, record.ttl, record.type, record.records[0].content)
    ...
    async def get_records(self, zone_id: str) -> list[DNSRRSetDTO]:
        """Get all DNS records."""
        if self._dns_settings.dns_server_ip is None or self._dns_settings.domain is None:
            raise DNSConnectionError
        ...
        if self._dns_settings.tsig_key is not None:
            query.use_tsig(keyring=TsigKey("zone.", self._dns_settings.tsig_key), keyname="zone.")

        await make_inbound_xfr(str(self._dns_settings.dns_server_ip), zone_tm)
    ...
    async def update_record(self, zone_id: str, record: DNSRRSetDTO) -> None:
        """Update DNS record."""
        action = Update(self._dns_settings.domain or zone_id)
        action.replace(record.name, record.ttl, record.type, record.records[0].content)
        await self._send(action)
    ...
    async def delete_record(self, zone_id: str, record: DNSRRSetDTO) -> None:
        """Delete DNS record."""
        action = Update(self._dns_settings.domain or zone_id)
        action.delete(record.name, record.type, record.records[0].content)
        await self._send(action)
    ...
    async def create_master_zone(self, zone: DNSMasterZoneDTO, is_empty: bool = False) -> None:  # noqa: ARG002
        raise DNSNotImplementedError

    async def create_forward_zone(self, zone: DNSForwardZoneDTO) -> None:  # noqa: ARG002
        raise DNSNotImplementedError

    async def update_master_zone(self, zone: DNSMasterZoneDTO) -> None:  # noqa: ARG002
        raise DNSNotImplementedError

    async def update_forward_zone(self, zone: DNSForwardZoneDTO) -> None:  # noqa: ARG002
        raise DNSNotImplementedError

    async def delete_master_zone(self, zone_id: str) -> None:  # noqa: ARG002
        raise DNSNotImplementedError

    async def delete_forward_zone(self, zone_id: str) -> None:  # noqa: ARG002
        raise DNSNotImplementedError
