...
class RIDSetGateway:
    ...
    async def get_rid_set_id_value(self) -> int:
        ...
        rid_set_id = await self._session.scalar(
            ...
            .select_from(domain)
            .join(domain_controllers_ou, qa(domain_controllers_ou.parent_id) == qa(domain.id))
            .join(domain_controller, qa(domain_controller.parent_id) == qa(domain_controllers_ou.id))
            .join(rid_set, qa(rid_set.parent_id) == qa(domain_controller.id))
            .where(
    ...
    async def get(self, domain_controller: Directory) -> Directory:
        ...
        rid_set = await self._session.scalar(
            select(Directory).where(qa(Directory.name) == "RID Set", qa(Directory.parent_id) == domain_controller.id)
        )
        if not rid_set:
            raise RIDManagerRidSetNotFoundError("RID Set directory not found")
    ...
    async def create_rid_set_directory(self, domain_controller: Directory) -> Directory:
        """Create RID Set directory."""
        rid_set_dir = Directory(is_system=True, name="RID Set")
        ...
        self._session.add(Attribute(name="cn", value="RID Set", directory_id=rid_set_dir.id))

        self._session.add(Attribute(name="objectClass", value="top", directory_id=rid_set_dir.id))

        self._session.add(Attribute(name="objectClass", value="rIDSet", directory_id=rid_set_dir.id))

        await self._session.flush()

        await self._session.refresh(instance=rid_set_dir, attribute_names=["attributes"], with_for_update=None)

        return rid_set_dir

    async def set_allocation_attrs(self, rid_set_id: int, allocation_params: RIDSetAllocationParamsDTO) -> None:
        """Set next RID attribute in RID Set directory."""
        self._session.add(Attribute(name="rIDNextRID", value=str(allocation_params.next_rid), directory_id=rid_set_id))
        self._session.add(
            Attribute(
        ...
        self._session.add(
            Attribute(name="rIDAllocationPool", value=str(allocation_params.allocation_pool), directory_id=rid_set_id)
        )
    ...
    async def get_rid_allocation_pool(self, rid_set_id: int) -> int:
        ...
        allocation_pool = await self._session.scalar(
            select(Attribute)
            .where(qa(Attribute.name) == "rIDAllocationPool", qa(Attribute.directory_id) == rid_set_id)
            .with_for_update()
        )
        if not (allocation_pool and allocation_pool.value):
            raise RIDManagerRidAllocationPoolNotFoundError("RID allocation pool not found")
        return int(allocation_pool.value)

    async def get_rid_previous_allocation_pool(self, rid_set_id: int) -> int:
        """Get previous RID allocation pool from RID Set directory."""
        previous_allocation_pool = await self._session.scalar(
            select(Attribute)
            .where(qa(Attribute.name) == "rIDPreviousAllocationPool", qa(Attribute.directory_id) == rid_set_id)
            .with_for_update()
        )
        if not (previous_allocation_pool and previous_allocation_pool.value):
            raise RIDManagerRidPreviousAllocationPoolNotFoundError("previous RID allocation pool not found")
    ...
    async def get_next_rid_value(self, rid_set_id: int) -> int:
        ...
        next_rid = await self._session.scalar(
            select(Attribute)
            .where(qa(Attribute.name) == "rIDNextRID", qa(Attribute.directory_id) == rid_set_id)
            .with_for_update()
        )
    ...
    async def update_next_rid(self, rid_set_id: int, next_rid: int) -> None:
        """Update next RID."""
    ...
    async def reset_attrs_when_pool_exceeded(
        self, rid_set_id: int, allocation_pool: int, previous_allocation_pool: int, next_rid: int
    ) -> None:
        ...
        await self.update_next_rid(rid_set_id, next_rid)
