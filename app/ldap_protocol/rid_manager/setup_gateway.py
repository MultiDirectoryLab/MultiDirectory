...
class RIDManagerSetupGateway:
    ...
    async def get_system_container(self) -> Directory:
        ...
        """
        query = select(Directory).where(qa(Directory.name) == SYSTEM_CONTAINER_NAME, qa(Directory.is_system).is_(True))

        system_container = await self._session.scalar(query)

        if not system_container:
            raise RIDManagerSystemContainerNotFoundError("System container not found")
    ...
    async def set_rid_manager(self) -> Directory:
        ...
        rid_manager_dir = Directory(is_system=True, name="RID Manager$")
        ...
        self._session.add(Attribute(name="cn", value="RID Manager$", directory_id=rid_manager_dir.id))

        self._session.add(Attribute(name="objectClass", value="top", directory_id=rid_manager_dir.id))

        self._session.add(Attribute(name="objectClass", value="rIDManager", directory_id=rid_manager_dir.id))

        await self._session.flush()

        await self._session.refresh(instance=rid_manager_dir, attribute_names=["attributes"], with_for_update=None)

        return rid_manager_dir

    async def set_rid_available_pool(self, rid_manager_dir: Directory, qword_value: int) -> None:
        ...
        """
        self._session.add(Attribute(directory_id=rid_manager_dir.id, name="rIDAvailablePool", value=str(qword_value)))

        await self._session.flush()

    def _generate_domain_sid_identifier(self) -> str:
        """Generate Domain Identifier for Active Directory domain."""
        return f"{secrets.randbits(32)}-{secrets.randbits(32)}-{secrets.randbits(32)}"
    ...
    async def create_domain_identifier(self, domain_id: int) -> None:
        ...
        domain_identifer = await self._session.scalar(
            select(exists(Attribute)).where(qa(Attribute.name) == "DomainIdentifier")
        )
        ...
        self._session.add(
            Attribute(
                name="DomainIdentifier", value=f"{self._generate_domain_sid_identifier()}", directory_id=domain_id
            )
        )
        await self._session.flush()
    ...
    async def get_domain_controller(self) -> Directory:
        ...
        domain_controller = await self._session.scalar(
            select(Directory).where(
                ...
                qa(Directory.parent).has(qa(Directory.name) == DOMAIN_CONTROLLERS_OU_NAME),
            )
        )

        if not domain_controller:
            raise RIDManagerDomainControllerNotFoundError("Domain controller not found")
