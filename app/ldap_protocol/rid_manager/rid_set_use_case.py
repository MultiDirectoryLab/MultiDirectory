...
class RIDSetUseCase:
    ...
    async def add(self, domain_controller: Directory, allocation_params: RIDSetAllocationParamsDTO) -> Directory:
        """Create RID Set directory."""
        rid_set = await self._gateway.create_rid_set_directory(domain_controller)
        await self._entity_type_use_case.attach_entity_type_to_directory(
            directory=rid_set, is_system_entity_type=True, object_class_names={"top", "rIDSet"}
        )

        await self._gateway.set_allocation_attrs(rid_set.id, allocation_params)
        await self._role_use_case.inherit_parent_aces(parent_directory=domain_controller, directory=rid_set)
        await self._session.flush()
        return rid_set

    def is_pool_exceeded(self, current_next_rid: int, previous_allocation_pool: int) -> bool:
    ...
    async def allocate_next_rid(self) -> int:
        ...
        async with self._session.begin_nested():
            ...
            current_next_rid = await self._gateway.get_next_rid_value(rid_set_id)
            previous_allocation_pool = await self._gateway.get_rid_previous_allocation_pool(rid_set_id)

            if not self.is_pool_exceeded(current_next_rid, previous_allocation_pool):
                new_next_rid = current_next_rid + 1
                await self._gateway.update_next_rid(rid_set_id, new_next_rid)
            else:
                new_next_rid = await self.rebind_next_rid_from_new_pool(rid_set_id)

        return new_next_rid

    async def rebind_next_rid_from_new_pool(self, rid_set_id: int) -> int:
        """Rebind next RID from new pool."""
        new_allocation_pool = await self._rid_manager_use_case.allocate_pool()

        current_allocation_pool = await self._gateway.get_rid_allocation_pool(rid_set_id)
    ...
    async def generate_rid_set_attrs(self) -> RIDSetAllocationParamsDTO:
        ...
        previous_allocation_pool = await self._rid_manager_use_case.allocate_pool()
