...
from ldap_protocol.utils import get_user, get_base_dn
...
async def authenticate_user(
    ...
    base_distinguished_name = await get_base_dn(session)
    if not user:
        return None
    if not verify_password(password, user.password):
        return None
    return User.from_db(user, access='access', base_distinguished_name=base_distinguished_name)
...
async def _get_user_from_token(
    ...
    return User.from_db(user, payload.get("grant_type"), base_distinguished_name, payload.get("exp"))
