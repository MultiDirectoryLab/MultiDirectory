...
def _convert_update_chema_to_dto(data: EntityTypeUpdateSchema) -> EntityTypeDTO:
    """Convert EntityTypeUpdateSchema to EntityTypeDTO."""
    return EntityTypeDTO(
        name=data.name, is_system=DEFAULT_ENTITY_TYPE_IS_SYSTEM, object_class_names=data.object_class_names
    )


_convert_request_to_dto = get_converter(EntityTypeSchema[None], EntityTypeDTO[None])

_convert_dto_to_schema = get_converter(EntityTypeDTO[int], EntityTypeSchema[int])


class LDAPEntityTypeFastAPIAdapter(
    BaseAdapter[EntityTypeUseCase],
    BaseLDAPSchemaAdapter[
        EntityTypeUseCase, EntityTypeSchema, EntityTypeUpdateSchema, EntityTypePaginationSchema, EntityTypeDTO
    ],
):
