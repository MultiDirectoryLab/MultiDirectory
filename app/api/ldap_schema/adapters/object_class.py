...
def _convert_update_schema_to_dto(request: ObjectClassUpdateSchema) -> ObjectClassDTO[None, str]:
...
_convert_schema_to_dto = get_converter(
    ...
    recipe=[
        link_function(lambda _: DEFAULT_OBJECT_CLASS_IS_SYSTEM, P[ObjectClassDTO].is_system),
        link_function(lambda _: None, P[ObjectClassDTO].id),
        link_function(lambda x: x.attribute_type_names_must, P[ObjectClassDTO].attribute_types_must),
        link_function(lambda x: x.attribute_type_names_may, P[ObjectClassDTO].attribute_types_may),
    ],
)
...
class ObjectClassFastAPIAdapter(
    ...
    BaseLDAPSchemaAdapter[
        ObjectClassUseCase, ObjectClassSchema, ObjectClassUpdateSchema, ObjectClassPaginationSchema, ObjectClassDTO
    ],
):
    """Object Class FastAPI Adapter."""
