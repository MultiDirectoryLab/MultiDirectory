...
from api.ldap_schema.schema import EntityTypePaginationSchema, EntityTypeSchema, EntityTypeUpdateSchema
from api.utils import require_master_db
from ldap_protocol.utils.pagination import PaginationParams


@ldap_schema_router.post(
    "/entity_type", status_code=status.HTTP_201_CREATED, error_map=error_map, dependencies=[Depends(require_master_db)]
)
async def create_one_entity_type(
    request_data: EntityTypeSchema[None], adapter: FromDishka[LDAPEntityTypeFastAPIAdapter]
) -> None:
...
async def get_one_entity_type(
    entity_type_name: str, adapter: FromDishka[LDAPEntityTypeFastAPIAdapter]
...
async def get_list_entity_types_with_pagination(
    adapter: FromDishka[LDAPEntityTypeFastAPIAdapter], params: Annotated[PaginationParams, Query()]
...
@ldap_schema_router.get("/entity_type/{entity_type_name}/attrs", error_map=error_map)
...
@ldap_schema_router.patch(
    "/entity_type/{entity_type_name}", error_map=error_map, dependencies=[Depends(require_master_db)]
)
async def modify_one_entity_type(
    entity_type_name: str, request_data: EntityTypeUpdateSchema, adapter: FromDishka[LDAPEntityTypeFastAPIAdapter]
) -> None:
    """Modify an Entity Type."""
    await adapter.update(name=entity_type_name, data=request_data)


@ldap_schema_router.post("/entity_type/delete", error_map=error_map, dependencies=[Depends(require_master_db)])
async def delete_bulk_entity_types(
    entity_type_names: LimitedListType, adapter: FromDishka[LDAPEntityTypeFastAPIAdapter]
) -> None:
