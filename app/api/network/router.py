...
from ldap_protocol.utils import get_groups, get_path_dn
...
async def add_network_policy(
    ...
    if policy.groups:
        ...
        group_dns = [
            get_path_dn(group.directory.path)
            for group in groups
        ]
...
async def update_network_policy(
    ...
    if request.groups is not None and len(request.groups) > 0:
        ...
        request.groups = [
            get_path_dn(group.directory.path) for group in groups]
    ...
    if request.mfa_groups is not None and len(request.mfa_groups) > 0:
        ...
        request.mfa_groups = [
            get_path_dn(group.directory.path) for group in mfa_groups]
