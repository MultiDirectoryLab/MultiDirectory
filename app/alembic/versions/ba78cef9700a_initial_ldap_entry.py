...
def calculate_entry(
    ...
    if {"top", "container"} == set(object_class_names):
        res.append("Группа (контейнер)")
    ...
    if {
        ...
        "posixAccount",
        "shadowAccount",
    }.issubset(object_class_names) and directory_name not in (
        "admin",
        "krbadmin",
    ):
    ...
    if {"krbContainer"}.issubset(
        object_class_names
    ) and directory_name == "kerberos":
        res.append("Керберос")
    if {"krbprincipal", "krbprincipalaux", "krbTicketPolicyAux"}.issubset(
        object_class_names
    ):
        res.append("KRB Принципал")
    if {"top", "krbrealmcontainer", "krbticketpolicyaux"}.issubset(
        object_class_names
    ):
        res.append("Realm Container")
...
def upgrade() -> None:
    ...
    entry_datas = (
        ...
        ("Группа (контейнер)", "groups", "top, container"),
        ...
        ("Группа", "group", "top, posixGroup, group"),
        ...
        ("Керберос", "kerberos", "krbContainer"),
        (
            "KRB Принципал",
            "",
            "krbprincipal, krbprincipalaux, krbTicketPolicyAux",
        ),
        ("Realm Container", "", "top, krbrealmcontainer, krbticketpolicyaux"),
    )
    ...
    async def _attach_entry_to_directories(connection):
        ...
        for directory in directories:
            ...
            if len(entry_names) > 1:
                raise Exception(
                    f"{directory} {directory.name} entry_names to long: {entry_names}"
                )

            if len(entry_names) == 0:
                # raise Exception(
                #     f"{directory} {directory.name} entry_names is empty: {entry_names}"
                # )
                continue
