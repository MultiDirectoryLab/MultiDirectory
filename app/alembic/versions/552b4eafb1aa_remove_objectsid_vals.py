...
from ldap_protocol.rid_manager.exceptions import RIDManagerNotFoundError, RIDManagerRidSetNotFoundError
...
async def _directory_ids_skipped_for_object_sid_migration(session: AsyncSession, domain: Directory) -> set[int]:
    ...
    peer_rows = await session.scalars(
        select(qa(Directory.id)).where(
            qa(Directory.parent_id) == domain.id, qa(Directory.name).in_(peer_container_names)
        )
    )
    skip_ids: set[int] = set(peer_rows.all())
    configuration_id = await session.scalar(
        select(qa(Directory.id)).where(
            qa(Directory.parent_id) == domain.id, qa(Directory.name) == CONFIGURATION_DIR_NAME
        )
    )
    if configuration_id is None:
        return skip_ids

    subtree = select(qa(Directory.id)).where(qa(Directory.id) == configuration_id).cte(name="subtree", recursive=True)
    subtree = subtree.union_all(select(qa(Directory.id)).where(qa(Directory.parent_id) == subtree.c.id))
...
def upgrade(container: AsyncContainer) -> None:  # noqa: C901
    ...
    async def _create_entity_types(connection: AsyncConnection) -> None:
        ...
        await entity_type_use_case.create(
            EntityTypeDTO(name=EntityTypeNames.RID_MANAGER, object_class_names=["top", "rIDManager"], is_system=True)
        )

        await entity_type_use_case.create(
            EntityTypeDTO(name=EntityTypeNames.RID_SET, object_class_names=["top", "rIDSet"], is_system=True)
        )
    ...
    async def _migrate_object_sids(connection: AsyncConnection) -> None:
        ...
        skip_object_sid_ids = await _directory_ids_skipped_for_object_sid_migration(session, domain)
        ...
        domain_sid_from_column = await session.scalar(
            select(directory_table.c.objectSid).where(directory_table.c.id == domain.id)
        )
        ...
        if domain_sid_from_column:
            ...
            if len(parts) >= 7 and domain_sid_from_column.startswith("S-1-5-21-"):
                domain_identifier = "-".join(parts[4:7])

        session.add(Attribute(name="DomainIdentifier", value=domain_identifier, directory_id=domain.id))
        result = (
            await session.execute(select(directory_table.c.id, directory_table.c.parentId, directory_table.c.objectSid))
        ).all()
        for directory_id, parent_id, object_sid in result:
            ...
            session.add(Attribute(name="objectSid", value=object_sid, directory_id=directory_id))

        sid_prefix = "S-1-5-21"
        for dir_name, rid in (
            (DOMAIN_ADMIN_GROUP_NAME, SecurityPrincipalRid.DOMAIN_ADMINS),
            (DOMAIN_USERS_GROUP_NAME, SecurityPrincipalRid.DOMAIN_USERS),
            (DOMAIN_COMPUTERS_GROUP_NAME, SecurityPrincipalRid.DOMAIN_COMPUTERS),
            ...
            await session.execute(
                ...
                .where(
                    ...
                    qa(Attribute.directory_id).in_(select(qa(Directory.id)).where(qa(Directory.name) == dir_name)),
                )
                .values(value=f"{sid_prefix}-{domain_identifier}-{int(rid)}")
            )
        ...
        await session.execute(
            ...
            .where(
                qa(Attribute.name) == "objectSid",
                qa(Attribute.value).like(f"S-1-5-21-%-{int(SecurityPrincipalRid.ADMINISTRATOR)}"),
            )
            .values(value=(f"{sid_prefix}-{domain_identifier}-{int(SecurityPrincipalRid.ADMINISTRATOR)}"))
        )
    ...
    async def _init_rid_manager(connection: AsyncConnection) -> None:
        ...
        domain_identifier = await session.scalar(
            select(Attribute).where(qa(Attribute.directory_id) == domain.id, qa(Attribute.name) == "DomainIdentifier")
        )
        ...
        sid_values = await session.scalars(
            select(Attribute).where(qa(Attribute.name) == "objectSid", qa(Attribute.value).like(f"{sid_prefix}%"))
        )
        ...
        await role_use_case.inherit_parent_aces(parent_directory=system_container, directory=rid_manager_dir)
        ...
        try:
            ...
            await rid_set_use_case.add(
                ...
                RIDSetAllocationParamsDTO(
                    next_rid=lower, allocation_pool=allocation_pool, previous_allocation_pool=previous_allocation_pool
                ),
            )
...
def downgrade(container: AsyncContainer) -> None:
    ...
    async def _delete_entity_types(connection: AsyncConnection) -> None:
        ...
        await session.execute(
            delete(EntityType).where(qa(EntityType.name).in_([EntityTypeNames.RID_MANAGER, EntityTypeNames.RID_SET]))
        )
    ...
    async def _delete_rid_manager_dirs(connection: AsyncConnection) -> None:
        ...
        await session.execute(delete(Directory).where(qa(Directory.name).in_(["RID Manager$", "RID Set"])))
        await session.commit()

    op.run_async(_delete_rid_manager_dirs)

    async def _rollback_object_sids(connection: AsyncConnection) -> None:
        ...
        for (directory_id,) in result:
            await session.execute(
                delete(Attribute).where(
                    qa(Attribute.directory_id) == directory_id, qa(Attribute.name) == "DomainIdentifier"
                )
            )
            ...
            await session.execute(
                update(directory_table).where(directory_table.c.id == directory_id).values(objectSid=attr.value)
            )

            await session.execute(
                delete(Attribute).where(qa(Attribute.directory_id) == directory_id, qa(Attribute.name) == "objectSid")
            )
